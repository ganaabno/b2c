...
export default function ManagerTourTable({ tours, loading, onEdit, onDelete }: TourTableProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "COMPLETED": return "bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400";
    ...
    <div className="overflow-hidden rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700">
      <table className="w-full text-left border-collapse">
        <thead className="bg-gray-50 dark:bg-gray-900/50 text-gray-600 dark:text-gray-300 uppercase text-sm font-semibold">
          <tr>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700">Аялал</th>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700">Departure</th>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700">Status</th>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700">Суудал</th>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700">Үнэ</th>
            <th className="px-2 py-4 border-b border-gray-200 dark:border-gray-700 text-right">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
          {loading ? (
            ...
            tours.map((tour) => (
              <tr key={tour.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                <td className="p-2 flex items-center gap-4">
                  ...
                  />
                  <div>
                    <div className="font-bold text-gray-900 dark:text-gray-100">{tour.title}</div>
                    <div className="text-xs text-gray-500 dark:text-gray-400">{tour.country}</div>
                  </div>
                </td>
                <td className="text-gray-700 dark:text-gray-300">
                  <div>{tour.departure_date ? tour.departure_date.split("T")[0] : ""}</div>
                </td>
                <td className="p-2">
