...
const deleteDalyan = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/dalyan/${id}`, {
    method: "DELETE",
...
const fetchDalyan = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/dalyan", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Dalyan price table");
  }
...
const updateDalyan = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/dalyan/${id}`, {
    method: "PUT",
    headers: {
...
const createDalyan = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/dalyan`, {
    method: "POST",
    headers: {
...
const DalyanTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["dalyanPrices"],
    queryFn: fetchDalyan,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateDalyan(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["dalyanPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createDalyan(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["dalyanPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteDalyan(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["dalyanPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Dalyan Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Dalyan Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Dalyan Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
