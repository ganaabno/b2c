...
const deleteTurkey = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/turkey/${id}`, {
    method: "DELETE",
...
const fetchTurkey = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/turkey", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Turkey price table");
  }
...
const updateTurkey = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/turkey/${id}`, {
    method: "PUT",
    headers: {
...
const createTurkey = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/turkey`, {
    method: "POST",
    headers: {
...
const TurkeyTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["turkeyPrices"],
    queryFn: fetchTurkey,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateTurkey(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["turkeyPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createTurkey(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["turkeyPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteTurkey(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["turkeyPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Turkey Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Turkey Row</h3>
              <button
                type="button"
                onClick={closeEdit}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-2 cursor-pointerrounded bg-amber-600 text-white">
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Turkey Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
            ...
            <input
              ...
              className="w-full cursor-pointer mb-3 px-3 py-2 rounded border"
            />
