...
const deleteThailandBanggok = async (id: string) => {
  ...
  const res = await fetch(`/api/price_table/thailand_banggok/${id}`, {
    method: "DELETE",
...
const fetchThailandBanggok = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/thailand_banggok", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Thailand price table");
  }
...
const updateThailandBanggok = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/thailand_banggok/${id}`, {
    method: "PUT",
    headers: {
...
const createThailandBanggok = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/thailand_banggok`, {
    method: "POST",
    headers: {
...
const ThailandBanggokTable = () => {
  const qc = useQueryClient();
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["thailandBanggokPrices"],
    queryFn: fetchThailandBanggok,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateThailandBanggok(id, body),
    onSuccess: () =>
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteThailandBanggok(id),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["thailandBanggokPrices"] }),
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Thailand Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Thailand Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
...
export default ThailandBanggokTable;
