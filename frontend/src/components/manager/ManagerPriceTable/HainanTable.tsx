...
const deleteHainan = async (id: string) => {
  ...
  const res = await fetch(`/api/price_table/hainan/${id}`, {
    method: "DELETE",
...
const fetchHainan = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/hainan", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Hainan price table");
  }
...
const updateHainan = async (id: string, body: Partial<Row>) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/hainan/${id}`, {
    method: "PUT",
    headers: {
...
const createHainan = async (body: Partial<Row>) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/hainan`, {
    method: "POST",
    headers: {
...
const HainanTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["hainanPrices"],
    queryFn: fetchHainan,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: Partial<Row> }) =>
      updateHainan(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["hainanPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: Partial<Row>) => createHainan(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["hainanPrices"] });
      setCreating(false);
      setNewRow({
        departure_date: "",
        five_star_adult_price: "",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteHainan(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["hainanPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Hainan Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Hainan Row</h3>
              <button
                type="button"
                onClick={closeEdit}
            ...
            <input
              type="number"
              inputMode="decimal"
              value={editing.child_two_to_five ?? ""}
              onChange={(e) =>
                setEditing({ ...editing, child_two_to_five: e.target.value })
              }
              className="w-full mb-3 px-3 py-2 rounded border"
            />
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Hainan Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
            ...
            <input
              type="number"
              inputMode="decimal"
              value={newRow.child_two_to_five}
              onChange={(e) =>
                setNewRow({ ...newRow, child_two_to_five: e.target.value })
              }
              className="w-full mb-3 px-3 py-2 rounded border"
            />
