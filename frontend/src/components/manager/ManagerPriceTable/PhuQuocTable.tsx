...
const deletePhuQuoc = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/phu_quoc/${id}`, {
    method: "DELETE",
...
const fetchPhuQuoc = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/phu_quoc", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch PhuQuoc price table");
  }
...
const updatePhuQuoc = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/phu_quoc/${id}`, {
    method: "PUT",
    headers: {
...
const createPhuQuoc = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/phu_quoc`, {
    method: "POST",
    headers: {
...
const PhuQuocTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["phuQuocPrices"],
    queryFn: fetchPhuQuoc,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updatePhuQuoc(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["phuQuocPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createPhuQuoc(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["phuQuocPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deletePhuQuoc(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["phuQuocPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">PhuQuoc Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit PhuQuoc Row</h3>
              <button
                type="button"
                onClick={closeEdit}
                className="p-1 text-gray-500 cursor-pointer hover:bg-gray-100 rounded">
                <X />
              </button>
            </div>
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add PhuQuoc Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
