...
const deleteHoChiMinhPhuQuoc = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/ho_chi_minh_phu_quoc/${id}`, {
    method: "DELETE",
...
const fetchHoChiMinhPhuQuoc = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/ho_chi_minh_phu_quoc", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(
      err.message || "Failed to fetch HoChiMinhPhuQuoc price table"
    );
  }
...
const updateHoChiMinhPhuQuoc = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/ho_chi_minh_phu_quoc/${id}`, {
    method: "PUT",
    headers: {
...
const createHoChiMinhPhuQuoc = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/ho_chi_minh_phu_quoc`, {
    method: "POST",
    headers: {
...
const HoChiMinhPhuQuocTable = () => {
  const qc = useQueryClient();
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["hoChiMinhPhuQuocPrices"],
    queryFn: fetchHoChiMinhPhuQuoc,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateHoChiMinhPhuQuoc(id, body),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["hoChiMinhPhuQuocPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createHoChiMinhPhuQuoc(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["hoChiMinhPhuQuocPrices"] });
      setCreating(false);
      setNewRow({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteHoChiMinhPhuQuoc(id),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["hoChiMinhPhuQuocPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">HoChiMinh/PhuQuoc Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit HoChiMinh/PhuQuoc Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add HoChiMinh/PhuQuoc Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-2 cursor-pointer rounded bg-amber-600 text-white">
...
export default HoChiMinhPhuQuocTable;
