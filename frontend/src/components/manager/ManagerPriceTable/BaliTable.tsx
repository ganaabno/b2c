...
const deleteBali = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/bali/${id}`, {
    method: "DELETE",
...
const fetchBali = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/bali", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Bali price table");
  }
...
const updateBali = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/bali/${id}`, {
    method: "PUT",
    headers: {
...
const createBali = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/bali`, {
    method: "POST",
    headers: {
...
const BaliTable = () => {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateBali(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["baliPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createBali(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["baliPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteBali(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["baliPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Bali Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
          ...
          <Plus className="h-4 w-4 cursor-pointer" /> Add Row
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Bali Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Bali Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
