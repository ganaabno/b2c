...
const deleteJapan = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/japan/${id}`, {
    method: "DELETE",
...
const fetchJapan = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/japan", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Japan price table");
  }
...
const updateJapan = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/japan/${id}`, {
    method: "PUT",
    headers: {
...
const createJapan = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/japan`, {
    method: "POST",
    headers: {
...
const JapanTable = () => {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateJapan(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["japanPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createJapan(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["japanPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteJapan(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["japanPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Japan Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Japan Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Japan Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
