...
const deleteNhaTrang = async (id: string) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/nha_trang/${id}`, {
    method: "DELETE",
...
const fetchNhaTrang = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/nha_trang", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Nha Trang price table");
  }
...
const updateNhaTrang = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/nha_trang/${id}`, {
    method: "PUT",
    headers: {
...
const createNhaTrang = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/nha_trang`, {
    method: "POST",
    headers: {
...
const NhaTrangTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["nhaTrangPrices"],
    queryFn: fetchNhaTrang,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updateNhaTrang(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["nhaTrangPrices"] }),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (body: any) => createNhaTrang(body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["nhaTrangPrices"] });
      setCreating(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteNhaTrang(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["nhaTrangPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Nha Trang Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Nha Trang Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Nha Trang Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
