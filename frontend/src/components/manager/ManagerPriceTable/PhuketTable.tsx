...
const deletePhuket = async (id: string) => {
  ...
  const res = await fetch(`/api/price_table/phuket/${id}`, {
    method: "DELETE",
...
const fetchPhuket = async (): Promise<Row[]> => {
  const token = localStorage.getItem("token");
  const res = await fetch("/api/price_table/phuket", {
    headers: token ? { Authorization: `Bearer ${token}` } : {},
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(err.message || "Failed to fetch Phuket price table");
  }
...
const updatePhuket = async (id: string, body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/phuket/${id}`, {
    method: "PUT",
    headers: {
...
const createPhuket = async (body: any) => {
  const token = localStorage.getItem("token");
  const res = await fetch(`/api/price_table/phuket`, {
    method: "POST",
    headers: {
...
const PhuketTable = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["phuketPrices"],
    queryFn: fetchPhuket,
  });
  const [editing, setEditing] = useState<Row | null>(null);
  const mutation = useMutation({
    mutationFn: ({ id, body }: { id: string; body: any }) =>
      updatePhuket(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["phuketPrices"] }),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deletePhuket(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["phuketPrices"] }),
  ...
  return (
    <div className="w-auto">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold">Phuket Price Table</h3>
        <button
          onClick={() => {
            setCreating(true);
            setNewRow({
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Edit Phuket Row</h3>
              <button
                type="button"
                onClick={closeEdit}
      ...
      {creating && (
        <div className="fixed inset-0 z-40 flex items-center justify-center">
          ...
          <form
            ...
            className="relative z-50 w-[600px] bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold">Add Phuket Row</h3>
              <button
                type="button"
                onClick={() => setCreating(false)}
                className="p-1 cursor-pointer text-gray-500 hover:bg-gray-100 rounded">
